...
import org.oscar.kb.keyboard.internal.keyboard_parser.floris.PopupSet
...
fun createPopupKeysArray(popupSet: PopupSet<*>?, params: _root_ide_package_.org.oscar.kb.keyboard.internal.KeyboardParams, label: String): Array<String>? {
    ...
    if (!popupKeysDelegate.isInitialized() || popupKeys.isEmpty())
        return null
    val fco = popupKeys.firstOrNull { it.startsWith(_root_ide_package_.org.oscar.kb.keyboard.Key.POPUP_KEYS_FIXED_COLUMN_ORDER) }
    if (fco != null && fco.substringAfter(_root_ide_package_.org.oscar.kb.keyboard.Key.POPUP_KEYS_FIXED_COLUMN_ORDER).toIntOrNull() != popupKeys.size - 1) {
        val fcoExpected = popupKeys.size - popupKeys.count { it.startsWith("!") && it.endsWith("!") } - 1
        if (fco.substringAfter(_root_ide_package_.org.oscar.kb.keyboard.Key.POPUP_KEYS_FIXED_COLUMN_ORDER).toIntOrNull() != fcoExpected)
    ...
    if (popupKeys.size > 1 && (label == "(" || label == ")")) { // add fixed column order for that case (typically other variants of brackets / parentheses
        ...
        popupKeys.clear()
        popupKeys.add("${_root_ide_package_.org.oscar.kb.keyboard.Key.POPUP_KEYS_FIXED_COLUMN_ORDER}${tmp.size}")
...
fun getHintLabel(popupSet: PopupSet<*>?, params: _root_ide_package_.org.oscar.kb.keyboard.internal.KeyboardParams, label: String): String? {
    ...
    return hintLabel?.let { _root_ide_package_.org.oscar.kb.keyboard.internal.KeySpecParser.getLabel(transformLabel(it, params)) }
...
private fun transformLabel(label: String, params: _root_ide_package_.org.oscar.kb.keyboard.internal.KeyboardParams): String =

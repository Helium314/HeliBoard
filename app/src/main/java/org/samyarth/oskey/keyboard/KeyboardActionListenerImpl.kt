...
import org.samyarth.oskey.latin.common.loopOverCodePoints
import org.samyarth.oskey.latin.common.loopOverCodePointsBackwards
...
class KeyboardActionListenerImpl(private val latinIME: LatinIME, private val inputLogic: InputLogic) :
    _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardActionListener {

    private val keyboardSwitcher =
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardSwitcher.getInstance()
    ...
    override fun onHorizontalSpaceSwipe(steps: Int): Boolean = when (Settings.getInstance().current.mSpaceSwipeHorizontal) {
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardActionListener.SWIPE_MOVE_CURSOR -> onMoveCursorHorizontally(steps)
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardActionListener.SWIPE_SWITCH_LANGUAGE -> onLanguageSlide(steps)
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardActionListener.SWIPE_TOGGLE_NUMPAD -> toggleNumpad(false, false)
        else -> false
    }

    override fun onVerticalSpaceSwipe(steps: Int): Boolean = when (Settings.getInstance().current.mSpaceSwipeVertical) {
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardActionListener.SWIPE_MOVE_CURSOR -> onMoveCursorVertically(steps)
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardActionListener.SWIPE_SWITCH_LANGUAGE -> onLanguageSlide(steps)
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardActionListener.SWIPE_TOGGLE_NUMPAD -> toggleNumpad(false, false)
        else -> false
    }
    ...
    override fun toggleNumpad(withSliding: Boolean, forceReturnToAlpha: Boolean): Boolean {
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardSwitcher.getInstance().toggleNumpad(withSliding, latinIME.currentAutoCapsState, latinIME.currentRecapitalizeState, forceReturnToAlpha)
        return true
    }
    ...
    private fun onLanguageSlide(steps: Int): Boolean {
        ...
        if (wantedIndex < 0)
            wantedIndex += subtypes.size
        _root_ide_package_.org.samyarth.oskey.keyboard.KeyboardSwitcher.getInstance().switchToSubtype(subtypes[wantedIndex])
        return true
    }

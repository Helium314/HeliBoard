...
import org.samyarth.oskey.compat.locale
...
import org.samyarth.oskey.dictionarypack.DictionaryPackConstants
import org.samyarth.oskey.keyboard.KeyboardLayoutSet
import org.samyarth.oskey.keyboard.KeyboardSwitcher
import org.samyarth.oskey.latin.common.Constants.Subtype.ExtraValue.KEYBOARD_LAYOUT_SET
import org.samyarth.oskey.latin.settings.Settings
import org.samyarth.oskey.latin.common.LocaleUtils
import org.samyarth.oskey.latin.common.LocaleUtils.constructLocale
import org.samyarth.oskey.latin.utils.*
import org.samyarth.oskey.latin.utils.ScriptUtils.script
import org.samyarth.oskey.latin.utils.CUSTOM_LAYOUT_PREFIX
import org.samyarth.oskey.latin.utils.DICTIONARY_URL
import org.samyarth.oskey.latin.utils.NewDictionaryAdder
import org.samyarth.oskey.latin.utils.POPUP_KEYS_LABEL_DEFAULT
import org.samyarth.oskey.latin.utils.POPUP_KEYS_ORDER_DEFAULT
import org.samyarth.oskey.latin.utils.ScriptUtils
import org.samyarth.oskey.latin.utils.addAdditionalSubtype
import org.samyarth.oskey.latin.utils.addEnabledSubtype
import org.samyarth.oskey.latin.utils.confirmDialog
import org.samyarth.oskey.latin.utils.createDictionaryTextHtml
import org.samyarth.oskey.latin.utils.displayName
import org.samyarth.oskey.latin.utils.editCustomLayout
import org.samyarth.oskey.latin.utils.getDictionaryLocales
import org.samyarth.oskey.latin.utils.getLayoutDisplayName
import org.samyarth.oskey.latin.utils.isAdditionalSubtype
import org.samyarth.oskey.latin.utils.loadCustomLayout
import org.samyarth.oskey.latin.utils.locale
import org.samyarth.oskey.latin.utils.onCustomLayoutFileListChanged
import org.samyarth.oskey.latin.utils.removeAdditionalSubtype
import org.samyarth.oskey.latin.utils.removeCustomLayoutFile
import org.samyarth.oskey.latin.utils.removeEnabledSubtype
import org.samyarth.oskey.latin.utils.reorderDialog
import org.samyarth.oskey.latin.utils.showMissingDictionaryDialog
import java.io.File
...
class LanguageSettingsDialog(
    ...
    private fun addSubtype(name: String) {
        ...
        val newSubtypeInfo = newSubtype.toSubtypeInfo(
            mainLocale,
            context,
            true,
    ...
    private fun copyLayout() {
        ...
        infos.forEach {
            ...
            if (layoutSetName?.startsWith(CUSTOM_LAYOUT_PREFIX) == false // don't allow copying custom layout (at least for now)
                && !layoutSetName.endsWith("+")
            ) { // don't allow copying layouts only defined via extra keys
        ...
        Builder(context)
            ...
            .setItems(displayNames.toTypedArray()) { di, i ->
                di.dismiss()
                val fileName =
                    context.assets.list("layouts")?.firstOrNull { it.startsWith(layouts[i]) }
                        ?: return@setItems
                loadCustomLayout(
                    context.assets.open("layouts${File.separator}$fileName").reader().readText(),
                    displayNames[i], mainLocale.toLanguageTag(), context
                ) { addSubtype(it) }
            }
            .setNegativeButton(android.R.string.cancel, null)
            .show()
    ...
    private fun addSubtypeToView(subtype: SubtypeInfo) {
        ...
        if (layoutSetName.startsWith(CUSTOM_LAYOUT_PREFIX)) {
            ...
            row.findViewById<View>(R.id.language_text)
                .setOnClickListener { editCustomLayout(layoutSetName, context) }
        } else {
                ...
                setOnClickListener {
                    ...
                    if (isCustom) {
                        confirmDialog(
                            context,
                            context.getString(
                                R.string.delete_layout,
                                getLayoutDisplayName(layoutSetName)
                            ),
        ...
        if (availableSecondaryLocales.isNotEmpty()) {
            binding.addSecondaryLanguage.apply {
                ...
                setOnClickListener {
                    ...
                    val localeNames =
                        locales.map { LocaleUtils.getLocaleDisplayNameInSystemLocale(it, context) }
                            .toTypedArray()
                    Builder(context)
    ...
    private fun addDictionaryToView(dictFile: File) {
        ...
        rowBinding.deleteButton.apply {
            ...
            setOnClickListener {
                confirmDialog(
                    context,
                    context.getString(R.string.remove_dictionary_message, dictType),
                    context.getString(
                        R.string.delete
                    )
                ) {
    ...
    private fun reloadDictionaries() = fragment?.activity?.sendBroadcast(
        Intent(
            DictionaryPackConstants.NEW_DICTIONARY_INTENT_ACTION
        )
    )
}
...
private fun getAvailableSecondaryLocales(
    context: Context,
    mainLocale: Locale,
    asciiCapable: Boolean
): Set<Locale> {

...
object LayoutUtils {
    ...
    fun getContent(layoutType: LayoutType, layoutName: String, context: Context): String {
        val layouts = context.assets.list(layoutType.folder.substringBeforeLast(File.separator))!!
        layouts.firstOrNull { it.startsWith("$layoutName.") }
            ?.let { return context.assets.open(layoutType.folder + it).reader().readText() }
        Log.e("LayoutUtils", "assets: ${context.assets.list("")?.toList()}")
        Log.e("LayoutUtils", "layouts: ${context.assets.list("layouts")?.toList()}")
        Log.e("LayoutUtils", "main layouts: ${context.assets.list("layouts/main")?.toList()}")

...
fun getCodeForToolbarKey(key: ToolbarKey) = Settings.getInstance().getCustomToolbarKeyCode(key) ?: when (key) {
    ...
    PASSWORD_MANAGER -> KeyCode.PASSWORD_MANAGER
}
...
enum class ToolbarKey {
    PASSWORD_MANAGER, VOICE, CLIPBOARD, NUMPAD, UNDO, REDO, SETTINGS, SELECT_ALL, SELECT_WORD, COPY, CUT, PASTE, ONE_HANDED, SPLIT,
...
val defaultToolbarPref by lazy {
    val default = listOf(SETTINGS, VOICE, CLIPBOARD, UNDO, REDO, SELECT_WORD, COPY, PASTE, LEFT, RIGHT, PASSWORD_MANAGER)
...
val defaultClipboardToolbarPref by lazy {
    ...
    val others = entries.filterNot { it in default || it == PASSWORD_MANAGER }
    default.joinToString(Separators.ENTRY) { it.name + Separators.KV + true } + Separators.ENTRY +
            others.joinToString(Separators.ENTRY) { it.name + Separators.KV + false }
}

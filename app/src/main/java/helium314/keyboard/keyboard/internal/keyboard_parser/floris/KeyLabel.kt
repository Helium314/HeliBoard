...
object KeyLabel {
    ...
    fun String.rtlLabel(params: KeyboardParams): String {
        val id = params.mId
        if (!id.subtype.isRtlSubtype || id.element.isNumberLayout) return this
        return when (this) {
            "{" -> "{|}"
            "}" -> "}|{"
            "(" -> "(|)"
    ...
    fun keyLabelToActualLabel(label: String, params: KeyboardParams): String {
        val newLabel = when (label) {
            SYMBOL_ALPHA -> if (params.mId.element.isAlphabetLayout) params.mLocaleKeyboardInfos.labelSymbol else params.mLocaleKeyboardInfos.labelAlphabet
    ...
    private fun getPeriodLabel(params: KeyboardParams): String {
        ...
        if (element.isNumberLayout) return "."
        if (element.isAlphabetLayout || id.locale.language in listOf("ar", "fa"))
    ...
    private fun getSpaceLabel(params: KeyboardParams): String {
        ...
        return if (element.isAlphaOrSymbolLayout || element.isBottomRow)
    ...
    private fun getActionKeyCode(params: KeyboardParams): String {
        val id = params.mId
        id.internalAction?.let { return "${KeyboardCodesSet.PREFIX_CODE}${it.code()}" }
        val element = id.element
        return if (id.isMultiLine && (element == KeyboardElement.ALPHABET_MANUAL_SHIFTED || element == KeyboardElement.ALPHABET_SHIFT_LOCK_SHIFTED))
    ...
    private fun getActionKeyLabel(params: KeyboardParams): String {
        val id = params.mId
        id.internalAction?.let { return it.label() }
        val element = id.element
        if (id.isMultiLine && (element == KeyboardElement.ALPHABET_MANUAL_SHIFTED || element == KeyboardElement.ALPHABET_SHIFT_LOCK_SHIFTED))
            return "!icon/enter_key"
        val iconName = when (id.imeAction()) {
            ...
            InputTypeUtils.IME_ACTION_CUSTOM_LABEL -> return id.customActionLabel
